...
impl Report {
    fn generate(
        ...
        let x = if f64::abs(owner_x - expected_x) > (owner_x.abs() + expected_x.abs()) / 1000. {
            Some(format!("Expected x: {expected_x}, actual x: {owner_x}"))
        } else {
            None
        };

        let owner_y = sprite.get_position().y as f64;
        let y = if f64::abs(owner_y - expected_y) > (owner_y.abs() + expected_y.abs()) / 1000. {
            Some(format!("Expected y: {expected_y}, actual y: {owner_y}"))
        } else {
            None
        };

        let owner_r = sprite.get_modulate().r as f64;
        let color = if f64::abs(owner_r - expected_r) > (owner_r.abs() + expected_r.abs()) / 1000. {
            Some(format!("Expected color r: {expected_r}, actual color r: {owner_r}"))
        } else {
            None
        };
...
impl Test {
    fn report(&self, tester: &mut Tester, sprite: &Gd<Node2D>, time: f64) {
        match self {
            Test::Absolute => {
                ...
                if let Some(report) =
                    Report::generate(sprite, time, expected_x, expected_y, expected_color_r)
                {
    ...
    fn start(&self, sprite: &Gd<Node2D>) -> RcPtr<SpireTween<Sequence>> {
        match self {
            ...
            Test::Relative => {
                sprite.do_color_r(1., D_2).register();
            ...
            Test::Delay => {
                sprite.do_color_r(0.5, D_2).with_delay(0.5).register();

                let mut seq = SpireTween::<Sequence>::new()
                    .bound_to(sprite.upcast())
                    .with_delay(3.0);
                seq.append(sprite.do_move_x(1000.0, D_1));
                seq.join(sprite.do_move_y(-1000.0, D_1));
...
impl INode2D for Tester {
    ...
    fn ready(&mut self) {
        ...
        buttons
            .get_node_as::<Button>("std")
            .connect_ex(
                "pressed",
                &Callable::from_object_method(&base, "_start_test")
                    .bindv(&Array::from(&[Test::Absolute.to_variant()])),
            )
            .flags(godot::classes::object::ConnectFlags::DEFERRED.ord() as u32)
            .done();

        buttons
            .get_node_as::<Button>("speed_based")
            .connect_ex(
                "pressed",
                &Callable::from_object_method(&base, "_start_test")
                    .bindv(&Array::from(&[Test::SpeedBased.to_variant()])),
            )
            .flags(godot::classes::object::ConnectFlags::DEFERRED.ord() as u32)
            .done();

        buttons
            .get_node_as::<Button>("relative")
            .connect_ex(
                "pressed",
                &Callable::from_object_method(&base, "_start_test")
                    .bindv(&Array::from(&[Test::Relative.to_variant()])),
            )
            .flags(godot::classes::object::ConnectFlags::DEFERRED.ord() as u32)
            .done();

        buttons
            .get_node_as::<Button>("delay")
            .connect_ex(
                "pressed",
                &Callable::from_object_method(&base, "_start_test")
                    .bindv(&Array::from(&[Test::Delay.to_variant()])),
            )
            .flags(godot::classes::object::ConnectFlags::DEFERRED.ord() as u32)
            .done();
    }
...
impl Tester {
    fn reset(&mut self) {
        self.reports.clear();
        let mut sprite = self.sprite.unwrap();
