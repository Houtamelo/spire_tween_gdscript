class_name TweenPropertyColor
const State = preload("res://handle/tween_handle.gd").State

var tween: InternalTweenPropertyColor

func _init(p_tween: InternalTweenPropertyColor):
	tween = p_tween
...
func from(value: Color) -> TweenPropertyColor:
	tween.set_start_value(value)
	return self

func with_ease(p_ease: Variant) -> TweenPropertyColor:
	tween = tween.with_ease(p_ease)
	return self

func set_absolute() -> TweenPropertyColor:
	tween.set_absolute()
	return self

func set_speed_based(speed: float) -> TweenPropertyColor:
	tween.set_speed_based(speed)
	return self

func set_relative(to: Color = Color(0, 0, 0, 0)) -> TweenPropertyColor:
