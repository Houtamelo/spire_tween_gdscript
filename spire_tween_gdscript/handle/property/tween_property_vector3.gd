class_name TweenPropertyVector3
const State = preload("res://handle/tween_handle.gd").State

var tween: InternalTweenPropertyVector3

func _init(p_tween: InternalTweenPropertyVector3):
	tween = p_tween
...
func from(value: Vector3) -> TweenPropertyVector3:
	tween.set_start_value(value)
	return self

func with_ease(p_ease: Variant) -> TweenPropertyVector3:
	tween = tween.with_ease(p_ease)
	return self

func set_absolute() -> TweenPropertyVector3:
	tween.set_absolute()
	return self

func set_speed_based(speed: float) -> TweenPropertyVector3:
	tween.set_speed_based(speed)
	return self

func set_relative(to: Vector3 = Vector3.ZERO) -> TweenPropertyVector3:
