class_name TweenPropertyString
const State = preload("res://handle/tween_handle.gd").State

var tween: InternalTweenPropertyString

func _init(p_tween: InternalTweenPropertyString):
	tween = p_tween
...
func from(value: String) -> TweenPropertyString:
	tween.set_start_value(value)
	return self

func with_ease(p_ease: Variant) -> TweenPropertyString:
	tween = tween.with_ease(p_ease)
	return self

func set_absolute() -> TweenPropertyString:
	tween.set_absolute()
	return self

func set_speed_based(speed: float) -> TweenPropertyString:
	tween.set_speed_based(speed)
	return self

func set_relative(to: String = "") -> TweenPropertyString:
