class_name TweenPropertyF64
const State = preload("res://handle/tween_handle.gd").State

var tween: InternalTweenPropertyF64

func _init(p_tween: InternalTweenPropertyF64):
	tween = p_tween
...
func from(value: float) -> TweenPropertyF64:
	tween.set_start_value(value)
	return self

func with_ease(p_ease: Variant) -> TweenPropertyF64:
	tween = tween.with_ease(p_ease)
	return self

func set_absolute() -> TweenPropertyF64:
	tween.set_absolute()
	return self

func set_speed_based(speed: float) -> TweenPropertyF64:
	tween.set_speed_based(speed)
	return self

func set_relative(to: float = 0.0) -> TweenPropertyF64:
