...
impl IPropertyData for SpotLight3DFloatData {
    ...
    fn get_property_path(&self) -> NodePath {
        match self.property {
            ...
            <SpotLight3DFloatKind>::SpotAngleAttenuation => NodePath::from("spot_angle_attenuation"),
...
impl TryFromPathAndObject for SpotLight3DFloatData {
    fn try_from_path_and_object(path: &str, object: Gd<Object>) -> Option<Self> {
        object.try_cast::<SpotLight3D>().ok().and_then(|owner| match path {
            "spot_angle" => Some(Self {
                property: <SpotLight3DFloatKind>::SpotAngle,
                owner_obj_or_node: ObjectOrNode::Node(owner.upcast()),
                owner,
            }),
            "spot_angle_attenuation" => Some(Self {
                property: <SpotLight3DFloatKind>::SpotAngleAttenuation,
                owner_obj_or_node: ObjectOrNode::Node(owner.upcast()),
                owner,
            }),
            "spot_attenuation" => Some(Self {
                property: <SpotLight3DFloatKind>::SpotAttenuation,
                owner_obj_or_node: ObjectOrNode::Node(owner.upcast()),
                owner,
            }),
            "spot_range" => Some(Self {
...
pub trait SpireDoSpotLight3D<Marker = ()> {
    fn do_spot_angle(&self, end_val: f64, duration: f64) -> SpireTween<LerpPropertyData<f64>>;
    fn do_spot_angle_attenuation(&self, end_val: f64, duration: f64) -> SpireTween<LerpPropertyData<f64>>;
    fn do_spot_attenuation(&self, end_val: f64, duration: f64) -> SpireTween<LerpPropertyData<f64>>;
    fn do_spot_range(&self, end_val: f64, duration: f64) -> SpireTween<LerpPropertyData<f64>>;
}
impl<Class: Inherits<SpotLight3D> + Inherits<Object>> SpireDoSpotLight3D<()> for Gd<Class> {
    fn do_spot_angle(&self, end_val: f64, duration: f64) -> SpireTween<LerpPropertyData<f64>> {
        let data = SpotLight3DFloatData {
            property: <SpotLight3DFloatKind>::SpotAngle,
            owner: self.clone().upcast(),
            owner_obj_or_node: ObjectOrNode::Node(self.clone().upcast::<SpotLight3D>().upcast::<Node>()),
        };
        SpireTween::<LerpPropertyData<f64>>::new(data.into(), Evaluator::Static(end_val), duration)
    }
    fn do_spot_angle_attenuation(&self, end_val: f64, duration: f64) -> SpireTween<LerpPropertyData<f64>> {
        let data = SpotLight3DFloatData {
            property: <SpotLight3DFloatKind>::SpotAngleAttenuation,
...
impl<T: WithBaseField + Inherits<SpotLight3D> + Inherits<Object>> SpireDoSpotLight3D<BaseMarker> for T {
    fn do_spot_angle(&self, end_val: f64, duration: f64) -> SpireTween<LerpPropertyData<f64>> {
