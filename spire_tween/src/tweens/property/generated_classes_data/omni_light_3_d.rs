...
impl TryFromPathAndObject for OmniLight3DFloatData {
    fn try_from_path_and_object(path: &str, object: Gd<Object>) -> Option<Self> {
        object.try_cast::<OmniLight3D>().ok().and_then(|owner| match path {
            "omni_attenuation" => Some(Self {
                property: <OmniLight3DFloatKind>::OmniAttenuation,
                owner_obj_or_node: ObjectOrNode::Node(owner.upcast()),
                owner,
            }),
            "omni_range" => Some(Self {
...
pub trait SpireDoOmniLight3D<Marker = ()> {
    fn do_omni_attenuation(&self, end_val: f64, duration: f64) -> SpireTween<LerpPropertyData<f64>>;
    fn do_omni_range(&self, end_val: f64, duration: f64) -> SpireTween<LerpPropertyData<f64>>;
}
impl<Class: Inherits<OmniLight3D> + Inherits<Object>> SpireDoOmniLight3D<()> for Gd<Class> {
    fn do_omni_attenuation(&self, end_val: f64, duration: f64) -> SpireTween<LerpPropertyData<f64>> {
        let data = OmniLight3DFloatData {
            property: <OmniLight3DFloatKind>::OmniAttenuation,
            owner: self.clone().upcast(),
            owner_obj_or_node: ObjectOrNode::Node(self.clone().upcast::<OmniLight3D>().upcast::<Node>()),
        };
...
impl<T: WithBaseField + Inherits<OmniLight3D> + Inherits<Object>> SpireDoOmniLight3D<BaseMarker> for T {
    fn do_omni_attenuation(&self, end_val: f64, duration: f64) -> SpireTween<LerpPropertyData<f64>> {
