...
impl TryFromPathAndObject for ConeTwistJoint3DFloatData {
    fn try_from_path_and_object(path: &str, object: Gd<Object>) -> Option<Self> {
        object.try_cast::<ConeTwistJoint3D>().ok().and_then(|owner| match path {
            "bias" => Some(Self {
                property: <ConeTwistJoint3DFloatKind>::Bias,
                owner_obj_or_node: ObjectOrNode::Node(owner.upcast()),
                owner,
            }),
            "relaxation" => Some(Self {
...
pub trait SpireDoConeTwistJoint3D<Marker = ()> {
    ...
    fn do_relaxation(&self, end_val: f64, duration: f64) -> SpireTween<LerpPropertyData<f64>>;
    fn do_softness(&self, end_val: f64, duration: f64) -> SpireTween<LerpPropertyData<f64>>;
    fn do_swing_span(&self, end_val: f64, duration: f64) -> SpireTween<LerpPropertyData<f64>>;
    fn do_twist_span(&self, end_val: f64, duration: f64) -> SpireTween<LerpPropertyData<f64>>;
}
impl<Class: Inherits<ConeTwistJoint3D> + Inherits<Object>> SpireDoConeTwistJoint3D<()> for Gd<Class> {
    fn do_bias(&self, end_val: f64, duration: f64) -> SpireTween<LerpPropertyData<f64>> {
        let data = ConeTwistJoint3DFloatData {
            property: <ConeTwistJoint3DFloatKind>::Bias,
            owner: self.clone().upcast(),
            owner_obj_or_node: ObjectOrNode::Node(self.clone().upcast::<ConeTwistJoint3D>().upcast::<Node>()),
        };
...
impl<T: WithBaseField + Inherits<ConeTwistJoint3D> + Inherits<Object>> SpireDoConeTwistJoint3D<BaseMarker> for T {
    fn do_bias(&self, end_val: f64, duration: f64) -> SpireTween<LerpPropertyData<f64>> {
