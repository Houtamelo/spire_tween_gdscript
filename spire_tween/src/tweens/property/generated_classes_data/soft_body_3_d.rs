...
impl TryFromPathAndObject for SoftBody3DFloatData {
    fn try_from_path_and_object(path: &str, object: Gd<Object>) -> Option<Self> {
        object.try_cast::<SoftBody3D>().ok().and_then(|owner| match path {
            "shrinking_factor" => Some(Self {
                property: <SoftBody3DFloatKind>::ShrinkingFactor,
                owner_obj_or_node: ObjectOrNode::Node(owner.upcast()),
                owner,
            }),
            "total_mass" => Some(Self {
...
pub trait SpireDoSoftBody3D<Marker = ()> {
    fn do_shrinking_factor(&self, end_val: f64, duration: f64) -> SpireTween<LerpPropertyData<f64>>;
    fn do_total_mass(&self, end_val: f64, duration: f64) -> SpireTween<LerpPropertyData<f64>>;
}
impl<Class: Inherits<SoftBody3D> + Inherits<Object>> SpireDoSoftBody3D<()> for Gd<Class> {
    fn do_shrinking_factor(&self, end_val: f64, duration: f64) -> SpireTween<LerpPropertyData<f64>> {
        let data = SoftBody3DFloatData {
            property: <SoftBody3DFloatKind>::ShrinkingFactor,
            owner: self.clone().upcast(),
            owner_obj_or_node: ObjectOrNode::Node(self.clone().upcast::<SoftBody3D>().upcast::<Node>()),
        };
...
impl<T: WithBaseField + Inherits<SoftBody3D> + Inherits<Object>> SpireDoSoftBody3D<BaseMarker> for T {
    fn do_shrinking_factor(&self, end_val: f64, duration: f64) -> SpireTween<LerpPropertyData<f64>> {
