...
pub fn generate_enum_constant_docs(raw_docs: &[&str], name: &str, value: i32) -> String {
    let docs_lines = raw_docs
        .iter()
        .map(|token_tree| {
            let str = token_tree.to_string();
            litrs::StringLit::parse(str.clone()).map_or(str, |parsed| parsed.value().to_string())
        })
        .collect::<Vec<_>>();

    let description_content = attribute_docs_to_xml_paragraphs(docs_lines);
    let name = xml_escape(name.to_string());

    format!(r#"<constant name="{name}" value="{value}">{description_content}</constant>"#)
}
...
/// Extracts docs from attributes
fn docs_with_attributes(docs_lines: Vec<String>) -> String {
...
fn attribute_docs_to_xml_paragraphs(docs_lines: Vec<String>) -> String {
    let docs = docs_with_attributes(docs_lines);
    let bb_code = to_bbcode(&docs);
    xml_escape(bb_code)
}
        ...
        let result = match node {
            ...
            List(md::List {
                ...
                for item_node in children.iter() {
                    if let ListItem(item) = item_node {
            ...
            TableCell(md::TableCell { children, .. }) => self.walk_inline_nodes(children, level),
